...
- 标签（tag）：表在 `internal/db`（`tags` 表带 `frozen` 列，`ALTER TABLE` 幂等迁移），
  ...
  **展示顺序可拖拽**：`tags` 表有 `sort_order`（`ALTER TABLE` 幂等迁移，回填 `sort_order = id`），
  `CreateTag` 取 `MAX(sort_order)+1`，`ListTags` 按 `frozen, sort_order, id` 排；
  行首的 `⠿` 手柄（`.tag-drag`）用指针事件自己实现（拖过的行记成落点、松手换位），
  松手后调 `App.ReorderTags(ids)`（一个事务里按数组下标写 `sort_order`，校验空数组/不存在的 id）
  再 `onChanged()` 拉回新顺序 —— 不依赖 HTML5 dnd，也不做乐观更新。
  **标签云**在同一个页面的第二个 tab（`tag-tab-list` / `tag-tab-cloud`）：
  字号 14~30px、不透明度 0.42~1 都按 `book_count / max(book_count)` 线性插值，
  文字用标签自己的颜色，点一下同样 `onOpenShelf([id])`。
  **云状排布**（不是按顺序铺排的列表）：`.tag-cloud` 用 `flex` + `justify-content/align-items: center`
  居中成一个团；标签顺序按数量降序后做 middle-out（偶数位放右半边、奇数位放左半边再倒序，
  最大的那个落在序列正中间，其余往两边变小）；每个标签再用 `Math.sin(id*…)` 取固定的 0~1 散列，
  换算成 ±10px 的 `--cloud-jitter` 交给 `transform: translateY()`（hover 缩放要写成
  `translateY(var(--cloud-jitter)) scale(1.06)`，否则会把抖动覆盖掉）。
...
- 书架多标签筛选有「或 / 且」两种匹配方式（`.tag-mode-select`，`data-testid="tag-mode"`，
  选项只有 `filter.or` / `filter.and`）：`or` 用一条 `EXISTS … tag_id IN (…)`（命中任意一个），
  `and` 仍是每个标签一条 `EXISTS`（必须全部命中）；前端 `BookQueryInput.tag_mode` → `db.BookQuery.TagMode`
  （常量 `db.TagFilterOr/TagFilterAnd`，空值按 `or` 处理）。
  **默认值来自设置**：设置页「书架 ‹ 标签匹配方式」存 `settings.tag_mode`（`config.Defaults` 里默认 `"or"`），
  `App.tsx` 用 `appliedTagModeSetting` 记住上次生效过的设置值 —— 设置一变（含首次加载）就覆盖书架当前选择，
  书架自己切换时不写设置、也不被冲掉。
- 书架筛选条最多直接显示 5 个标签（`TAG_CHIP_LIMIT`），超出的收进「⋯」按钮
  （`.chip-more`，`data-testid="tag-more"`，`title`/`aria-label` 都是 `filter.moreTagsTip`「选择更多标签」，
  按钮上带被收起的数量）；点它开 `tag-more-modal` 多选弹窗（列出全部标签含冻结项、带已选计数、
  「清空选择」/「取消」/「应用筛选」），应用后走同一个 `onTagFilter`。
  已经在筛选里的标签即使排在第 6 个以后也仍然留在筛选条上（否则看不到筛选条件）。
  筛选条上的标签**不显示书籍数量**（`⋯` 按钮上的数字是「被收起的标签个数」，保留）；
  弹窗里的标签也**不带数量**，而是平铺成网格 `.tag-pick-grid`（`repeat(auto-fill, minmax(118px, 1fr))`，
  每个是 `.pick-tile` 方块，选中加 `.active` + 右侧 `✓`），`max-height: 260px` + `overflow-y: auto`，
  标签多了自己出滚动条。
...
- UI 改动后跑 `just ui-test`：它用 playwright-core 加载 `dist/` 并对 `window.go` 打桩，覆盖书架（含搜索框清空按钮与 Esc 清空、滚动位置恢复、多标签筛选的或/且切换与查询参数、筛选条最多直显 5 个（标签不带数量）+「⋯」多选弹窗（平铺网格 + 滚动 + 也不带数量）、批量管理：勾选/全选/批量打标签三种方式/批量删除与取消/冻结标签不进选择器）/统计/扫描/标签页（侧栏与工具卡片两个入口、书架工具栏没有入口、新建/随机颜色/改名换色/冻结解冻/删除、拖拽排序调用参数与顺序落库、标签云 tab 的居中云状排布/多行/上下错落/最大的落在中间/字号 14~30 与不透明度随数量变化、点数量跳筛选）、误录管理页（统计页入口与工具卡片都进整页、逐条恢复、全部清除后回到空状态）/设置（含标签匹配方式默认值 → 书架跟随、书架手选不写设置）/书籍详情/EPUB 阅读器（当前字号显示、字号右侧的护眼模式开关、字号改动后工具栏数字同步）与加密 PDF 阅读器（含护眼模式的页面滤镜）/工具页（分类分组 + 类型筛选）与 PDF 设置·清除密码·转存 EPUB·转存 PDF·合并 PDF·提取页面·转存图片·修改文档·压缩文档 弹窗（含书架右键 EPUB 工具子菜单、合并列表顺序调整与加密文件密码、提取页面的 20 页分组/跨组选择/放大查看、转存图片的页码范围解析/DPI 像素数/JPEG 质量与中途停止、修改文档的原值预填/改动汇总与还原/另存与覆盖两种保存方式/加密文件密码流程、压缩文档的 Ghostscript 检测与手动指定/档位与自定义分辨率/自动退回 pdfcpu/加密文件密码流程）。
