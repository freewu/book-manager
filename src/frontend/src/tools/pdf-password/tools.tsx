// 设置密码 —— PDF 工具弹窗。
//
// 入口：工具页「PDF › 设置密码」选本地文件；书架右键 PDF 书籍 → PDF 工具 → 设置密码。
// 已加密的 PDF 需要先输入当前密码，才能换成新密码。
import React, {useCallback, useEffect, useState} from 'react';
import type {PdfFileInfo} from '../../types';
import {humanSize} from '../../api';
import {useI18n} from '../../i18n';
import type {ToolDialogProps} from '../types';
import {inspectPdf, pickPdfFile, setPdfPassword} from './lib';

const STRENGTHS = [
  {key: 'aes256', labelKey: 'tools.pdfPwAes256'},
  {key: 'aes128', labelKey: 'tools.pdfPwAes128'},
  {key: 'rc4128', labelKey: 'tools.pdfPwRc4'},
];

export default function PdfPasswordToolDialog({book, onClose, onChanged}: ToolDialogProps) {
  const {t} = useI18n();
  ...
  const [strength, setStrength] = useState('aes256');
  ...
  const inspect = useCallback(async (p: string, pw = '') => {
    setErr('');
    try {
  ...
  const submit = async () => {
    ...
    if (!userPw) {
      setErr(t('tools.pdfPwErrEmpty'));
      return;
    }
    if (userPw !== userPw2) {
      setErr(t('tools.pdfPwErrMismatch'));
      return;
    }
    ...
    setErr('');
    try {
      const res = await setPdfPassword({
        book_id: bookId ?? 0,
        path,
      ...
      <div className="modal" style={{width: 560}} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          <div className="form-row">
            ...
            {book && bookId !== undefined && <div className="hint">{t('tools.pdfPwFromShelf', {title: book.title || book.file_name})}</div>}
          </div>
          ...
          <div className="form-row">
            <label>{t('tools.pdfPwNew')}</label>
            <input type="password" value={userPw} onChange={(e) => setUserPw(e.target.value)} disabled={busy} />
          </div>

          <div className="form-row">
            <label>{t('tools.pdfPwConfirm')}</label>
            <input type="password" value={userPw2} onChange={(e) => setUserPw2(e.target.value)} disabled={busy} />
          </div>

          <div className="form-row">
            <label>{t('tools.pdfPwOwner')}</label>
            <input
              type="password"
              value={ownerPw}
              placeholder={t('tools.pdfPwOwnerPlaceholder')}
          ...
          <div className="form-row">
            <label>{t('tools.pdfPwStrength')}</label>
            <div className="chip-row">
              {STRENGTHS.map((s) => (
                <button
                  key={s.key}
                  className={`chip ${strength === s.key ? 'active' : ''}`}
          ...
          <div className="form-row">
            ...
            <label style={{display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', marginTop: 6}}>
              <input
                ...
                disabled={busy}
              />
              {t('tools.pdfPwAllowCopy')}
            </label>
          </div>

          {err && <div className="tool-note err">{err}</div>}
          {done && <div className="tool-note ok">✅ {t('tools.pdfPwDone')}</div>}
          {!done && <div className="hint">{t('tools.pdfPwHint')}</div>}
        </div>
        <div className="modal-foot">
          ...
          {!done && (
            <button className="btn btn-primary" onClick={submit} disabled={busy || !path}>
              {busy ? t('tools.pdfPwWorking') : t('tools.pdfPwSubmit')}
            </button>
          )}
        </div>
      </div>
